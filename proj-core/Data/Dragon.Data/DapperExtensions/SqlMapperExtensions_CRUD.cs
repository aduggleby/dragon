...
namespace Dapper
{
    ...
    public static partial class SqlMapperExtensions
    {
        ...
        private static void Log(string sql, object o, ILogger logger)
        {
            if (logger!=null)
            {
                ...
                logger.LogTrace("{0} - Params: {1}{2}", sql, o == null ? "<none>" : sb.ToString().TrimEnd(','), Environment.NewLine);
            }
        }


        public static T Get<T>(this IDbConnection connection, dynamic id, IDbTransaction transaction = null, int? commandTimeout = null, ILogger logger = null) where T : class
        {
            var type = typeof(T);
            ...
            T obj = null;
            Log(sql, dynParms, logger);
            obj = connection.Query<T>(sql, dynParms, transaction: transaction, commandTimeout: commandTimeout).FirstOrDefault();
            return obj;
        }


        public static IEnumerable<T> GetList<T>(this IDbConnection connection, IEnumerable<dynamic> ids, IDbTransaction transaction = null, int? commandTimeout = null, ILogger logger = null) where T : class
        {
            var type = typeof(T);
            var parameters = new Dictionary<string, object>();
        ...
        public static IEnumerable<T> GetAll<T>(this IDbConnection connection, IDbTransaction transaction = null, int? commandTimeout = null, ILogger logger = null) where T : class
        {
            var type = typeof(T);
            string sql;
        ...
        public static TKey Insert<T, TKey>(this IDbConnection connection,
            ...
            bool letDbGenerateKey = false, ILogger logger = null) where T : class
        {
            var type = typeof(T);
            var metadata = MetadataFor(type);
        ...
        public static bool Update<T>(this IDbConnection connection, T entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null, ILogger logger = null) where T : class
        {
            var type = typeof(T);
            var metadata = MetadataFor(type);
        ...
        public static bool Delete<T>(this IDbConnection connection, T entityToDelete, IDbTransaction transaction = null, int? commandTimeout = null, ILogger logger = null) where T : class
        {
            var type = typeof(T);
            var metadata = MetadataFor(type);
            ...
            Log(sql, entityToDelete, logger);

            var deleted = connection.Execute(sql, entityToDelete, transaction: transaction, commandTimeout: commandTimeout);
            return deleted > 0;
        }

        public static bool Exists<T>(this IDbConnection connection, T entity, IDbTransaction transaction = null, int? commandTimeout = null, ILogger logger = null) where T : class
        {
            var type = typeof(T);
            var metadata = MetadataFor(type);
